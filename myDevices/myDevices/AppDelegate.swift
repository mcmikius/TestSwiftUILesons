...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        do {
            if let results = try persistentContainer.viewContext.execute(fetchRequest) as? [NSManagedObject] {
    ...
    func addTestData() {
        guard let entity = NSEntityDescription.entity(forEntityName: "Device", in: persistentContainer.viewContext) else { fatalError("Could not find entity description!") }
        for i in 1...25 {
